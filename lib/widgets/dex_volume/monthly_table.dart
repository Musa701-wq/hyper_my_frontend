...
class MonthlyVolumeTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    sortedTrend.sort((a, b) => b.toDateTime().compareTo(a.toDateTime())); // Newest first
    
    final last12Months = sortedTrend.take(12).toList();
    final double maxVol = monthlyTrend.isEmpty ? 0 : monthlyTrend.map((e) => e.volume).reduce((a, b) => a > b ? a : b);
          ...
          ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final prevMonth = monthlyTrend.firstWhere(
                (e) => e.toDateTime().isAtSameMomentAs(DateTime.utc(date.year, date.month - 1, 1)),
                orElse: () => TrendItem(label: '', volume: 0),
              );
              if (prevMonth.volume > 0) {
                momChange = ((item.volume - prevMonth.volume) / prevMonth.volume) * 100;
              }
              ...
              final bool isCurrentMonth = date.month == DateTime.now().month && date.year == DateTime.now().year;
                  ...
                  children: [
                    Expanded(
                      ...
                      child: Text(
                        item.label,
                        style: GoogleFonts.jetBrainsMono(
                          color: isCurrentMonth ? const Color(0xFF10B981) : Colors.white, 
                          fontSize: 11, 
                          fontWeight: FontWeight.bold
                        ),
                      ),
                    ...
                    Expanded(
                      ...
                      child: Text(
                         ...
                         textAlign: TextAlign.right,
                         style: GoogleFonts.jetBrainsMono(
                           color: (momChange ?? 0) >= 0 ? const Color(0xFF10B981) : const Color(0xFFF43F5E),
                           fontSize: 10,
                           fontWeight: FontWeight.bold,
                         ),
                      ),
                    ...
                    Expanded(
                      ...
                      child: Text(
                         ...
                         textAlign: TextAlign.right,
                         style: GoogleFonts.jetBrainsMono(
                           color: (vsAvg ?? 0) >= 0 ? const Color(0xFF10B981) : const Color(0xFFF43F5E),
                           fontSize: 10,
                         ),
                      ),
                    ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(flex: 3, child: Text('VOLUME', textAlign: TextAlign.right, style: GoogleFonts.jetBrainsMono(color: AppColors.textSecondary, fontSize: 9, fontWeight: FontWeight.bold))),
          Expanded(flex: 2, child: Text('MOM', textAlign: TextAlign.right, style: GoogleFonts.jetBrainsMono(color: AppColors.textSecondary, fontSize: 9, fontWeight: FontWeight.bold))),
          Expanded(flex: 2, child: Text('VS 6M AVG', textAlign: TextAlign.right, style: GoogleFonts.jetBrainsMono(color: AppColors.textSecondary, fontSize: 9, fontWeight: FontWeight.bold))),
        ],
      ),
    );
  }
